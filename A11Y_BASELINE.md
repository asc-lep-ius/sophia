...
- The gate target is zero serious, critical, or structural axe violations. New route work adds that route to the same matrix in the commit that adds the route; with NiceGUI retired there is no second surface to fall back to.
...
- `frontend/tests/e2e/long-tail.spec.ts` carries the same two widths for search, chronos, chronos history, calibration and register, alongside each page's happy and error path. The long-tail surfaces are not high-traffic, so they are gated in their own spec rather than added to the one above; the reflow requirement is identical.
...
- Progressive enhancement: a write that a learner has to complete — the content upload, the deadline sync, and a TISS registration — works as a plain form post. Client enhancement may add progress, cancellation or a retry, never the only working path. `frontend/tests/e2e/content-topics.spec.ts` runs that upload with JavaScript switched off, and `frontend/tests/e2e/long-tail.spec.ts` does the same for the sync, the registration and a submitted search.
