...
def test_blocking_audit_recurses_nested_router_packages(tmp_path: Path) -> None:
    routers_dir = tmp_path / "src" / "sophia" / "api" / "routers" / "study"
    routers_dir.mkdir(parents=True)
    (routers_dir / "bad.py").write_text(
        textwrap.dedent(
            """
    ...
    assert result.returncode == 1
    assert "src/sophia/api/routers/study/bad.py" in result.stderr
    assert "Path.read_text" in result.stderr
